      ...
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          ...
          if (user.pickaxe > 0) return m.reply('You already have this')
            if(user.rock < 5 || user.wood < 10 || user.iron < 5 || user.string < 20) return m.reply(`Not enough stuff!\nTo make a pickaxe. You need : \n10 wood🪵 \n5 iron⛓\n20 String🕸️\n5 Stone 🪨`)
          ...
          if (user.sword > 0) return m.reply('You already have this')
            if(user.wood < 10 || user.iron < 15) return m.reply(`Not enough items!\To make a sword. You need:\n10 wood🪵\n15 iron⛓️`)
            global.db.data.users[m.sender].wood -= 10
          ...
          if (user.fishingrod > 0) return m.reply('You already have this')
            if(user.wood < 20 || user.iron < 5 || user.string < 20) return m.reply(`Not enough goods!\To make a fishing rod. You need:\n10 wood🪵\n5 iron⛓\n20 String🕸️`)
            global.db.data.users[m.sender].wood -= 10
            ...
            break
          case 'armor':
          if (user.armor > 0) return m.reply('You already have this')
            if(user.iron < 30 || user.emerald < 1 || user.diamond < 5) return m.reply(`Not enough stuff!\nTo make armor. You need :\n30 Iron ⛓️\n1 Emerald ❇️\n5 Diamond 💎`)
